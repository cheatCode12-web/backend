...
if (
  pool &&
  typeof warmCoreSchema === "function" &&
  (process.env.RUN_MIGRATIONS === "true" || process.env.RUN_DB_INIT === "true")
) {
  // Only run schema warmup when explicitly requested via env vars
  void warmCoreSchema();
} else {
  console.log("Schema warmup skipped (RUN_MIGRATIONS/RUN_DB_INIT not enabled)");
}
